  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-300">Welcome back! Here's what's happening with your movies.</p>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((stat, index) => (
          <div key={index} className="rounded-lg p-6 border" style={{ backgroundColor: 'var(--brand-black)', borderColor: 'var(--navy)' }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-300 text-sm">{stat.label}</p>
                <p className="text-2xl font-bold text-white">{stat.value}</p>
              </div>
              <div className="flex-shrink-0" style={{ color: 'var(--brand-orange)' }}>
                {stat.icon}
              </div>
            </div>
          </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="font-medium hover:underline" style={{ color: 'var(--brand-orange)' }}>
            View All
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {featuredMovies.map((movie) => (
            <div key={movie.id} className="rounded-lg overflow-hidden border hover:border-opacity-80 transition-colors" style={{ backgroundColor: 'var(--brand-black)', borderColor: 'var(--navy)' }}>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button className="hover:opacity-80 transition-opacity" style={{ color: 'var(--brand-orange)' }}>
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                  </button>
                </div>
                <p className="text-gray-400 text-xs mt-2">{movie.genre}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mb-8">
        ...
        <div className="rounded-lg border" style={{ backgroundColor: 'var(--brand-black)', borderColor: 'var(--navy)' }}>
          <div className="p-6">
            <div className="space-y-4">
              {featuredMovies.slice(0, 3).map((movie) => (
                <div key={movie.id} className="flex items-center space-x-4 p-4 rounded-lg" style={{ backgroundColor: 'var(--navy)' }}>
                  <img
                    src={movie.image}
                    alt={movie.title}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center mt-2">
                      ...
                      <span className="text-gray-400 text-sm ml-4">Watched 2 days ago</span>
                    </div>
                  </div>
